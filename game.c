#include "game.h"
#include <stdlib.h>
#include <string.h>
#include <math.h>
#include <time.h>
#include "font.h"      // определим в этом же файле или отдельно? Упростим: вставим font сюда же.

// Встроим font (stb_truetype) и texture загрузку для простоты
#define STB_TRUETYPE_IMPLEMENTATION
#include "stb_truetype.h"
#define STB_IMAGE_IMPLEMENTATION
#include "stb_image.h"

typedef struct { uint32_t* data; int w,h; } Texture;
typedef struct { stbtt_fontinfo info; float scale; unsigned char* buf; } Font;

static Texture* load_texture(AAssetManager* mgr, const char* path){
    AAsset* a=AAssetManager_open(mgr,path,AASSET_MODE_BUFFER);
    if(!a) return NULL;
    size_t sz=AAsset_getLength(a);
    unsigned char* d=malloc(sz);
    AAsset_read(a,d,sz); AAsset_close(a);
    int w,h,n; unsigned char* img=stbi_load_from_memory(d,sz,&w,&h,&n,4);
    free(d);
    if(!img) return NULL;
    Texture* t=malloc(sizeof(Texture));
    t->w=w; t->h=h; t->data=malloc(w*h*4);
    for(int i=0;i<w*h;i++){ uint8_t r=img[i*4],g=img[i*4+1],b=img[i*4+2],a=img[i*4+3]; t->data[i]=(a<<24)|(r<<16)|(g<<8)|b; }
    stbi_image_free(img);
    return t;
}
static void free_texture(Texture* t){ if(t){ free(t->data); free(t); } }

static Font* font_load(AAssetManager* mgr, const char* path, float height){
    AAsset* a=AAssetManager_open(mgr,path,AASSET_MODE_BUFFER);
    if(!a) return NULL;
    size_t sz=AAsset_getLength(a);
    unsigned char* d=malloc(sz);
    AAsset_read(a,d,sz); AAsset_close(a);
    Font* f=malloc(sizeof(Font));
    f->buf=d;
    if(!stbtt_InitFont(&f->info, f->buf, 0)){ free(d); free(f); return NULL; }
    f->scale=stbtt_ScaleForPixelHeight(&f->info, height);
    return f;
}
static void font_free(Font* f){ if(f){ free(f->buf); free(f); } }
static void font_draw_text(Font* f, RenderBuffer* rb, int x, int y, const char* text, uint32_t color){
    if(!f || !rb || !text) return;
    int cx=x;
    for(const char* p=text; *p; ++p){
        int gi=stbtt_FindGlyphIndex(&f->info, *p);
        if(!gi){ cx+=8; continue; }
        int x0,y0,x1,y1; stbtt_GetGlyphBitmapBox(&f->info, gi, f->scale, f->scale, &x0,&y0,&x1,&y1);
        int w=x1-x0, h=y1-y0;
        if(w>0 && h>0){
            unsigned char* bm=malloc(w*h);
            if(bm){
                stbtt_MakeGlyphBitmap(&f->info, bm, w, h, w, f->scale, f->scale, gi);
                for(int row=0; row<h; row++){
                    int sy=y+y0+row;
                    if(sy<0 || sy>=rb->height) continue;
                    uint32_t* line=rb->pixels+sy*rb->stride;
                    for(int col=0; col<w; col++){
                        unsigned char a=bm[row*w+col];
                        if(a){ int sx=cx+x0+col; if(sx>=0 && sx<rb->width) line[sx]=color; }
                    }
                }
                free(bm);
            }
        }
        int adv; stbtt_GetGlyphHMetrics(&f->info, gi, &adv,0);
        cx += (int)(adv * f->scale);
    }
}

// Game state
static struct {
    Texture* playerTex;
    Texture* spikeTex;
    Font* font;
    float px, py, vy, size, angle, targetAngle, jumpTime, jumpDur;
    int grounded, jumpCount, gameOver, initialized, score;
    float groundY, gravity, jumpPower, speed;
    struct { float x,y; int active; } spikes[50];
    int spikeCount, spawnTimer, w,h;
} G;

void game_init(AAssetManager* assets, int w, int h){
    memset(&G,0,sizeof(G));
    G.w=w; G.h=h;
    G.groundY = h - 120;
    G.gravity = 0.6f; G.jumpPower = -12.0f; G.speed = 7.0f;
    G.jumpDur = 0.48f;
    G.playerTex = load_texture(assets, "cube1.png");
    G.spikeTex = load_texture(assets, "spike.png");
    G.font = font_load(assets, "Roboto-Regular.ttf", h/25.0f);
    G.size = 150 * 0.35f;
    G.px = 200;
    G.py = G.groundY - G.size/2;
    G.grounded = 1;
    G.initialized = 1;
    srand(time(NULL));
    for(int i=0;i<5;i++){ if(G.spikeCount<50){ G.spikes[G.spikeCount].x=G.w+50+rand()%300; G.spikes[G.spikeCount].y=G.groundY; G.spikes[G.spikeCount].active=1; G.spikeCount++; } }
}
int game_is_initialized(){ return G.initialized; }
void game_free(){ if(G.playerTex) free_texture(G.playerTex); if(G.spikeTex) free_texture(G.spikeTex); if(G.font) font_free(G.font); memset(&G,0,sizeof(G)); }

void game_jump(){
    if(G.gameOver){ game_init(G.assets? but we don't have assets pointer here... fix: store assets in global) }
}
// Из-за отсутствия assets в глобальной структуре – переделаем: сохраним asset manager в глобал.
